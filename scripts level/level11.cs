...
IEnumerator playNote(int counter,AudioSource C,AudioSource D,AudioSource E,AudioSource F,AudioSource G,AudioSource A,AudioSource B,double time){
    ...
    async void Start()
    {
        ...
        StartCoroutine(playNote(arr[1],SC,SD,SE,SF,SG,SA,SB,0.5));
        StartCoroutine(playNote(arr[2],SC,SD,SE,SF,SG,SA,SB,1));
        StartCoroutine(playNote(arr[3],SC,SD,SE,SF,SG,SA,SB,1.5));
        StartCoroutine(playNote(arr[4],SC,SD,SE,SF,SG,SA,SB,2));
                StartCoroutine(playNote(arr[5],SC,SD,SE,SF,SG,SA,SB,2.5));
                        StartCoroutine(playNote(arr[6],SC,SD,SE,SF,SG,SA,SB,3));
    ...
    AudioSource C,AudioSource D,AudioSource E,AudioSource F,AudioSource G,AudioSource A,AudioSource B)
    {
        ...
        if(val == order[counter]){
            ...
            point+=10;
            Score.text = "Score = "+point;
            if(point == 70 )SceneManager.LoadScene("G12");
